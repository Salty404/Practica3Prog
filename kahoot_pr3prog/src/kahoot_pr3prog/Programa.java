...
public class Programa {
		...
		do { // Bucle para que puedas operar en el programa tantas veces como quieras
			...
			if (opcion == 1) {
				...
				do { // Bucle para que puedas jugar tantas veces como quieras
					...
					for (int i = 0; i < idpreguntas.size(); i++) {
						...
						if (idrespuestas.contains(idcorrecto)) { // Reviso si en las 3 respuestas generadas
																	// aleatoriamente está la correcta

							idrespuestas.add(bd.idUltimoRegistro(false)); // Esta respuesta no se puede insertar
																			// aleatoriamente, si ya está la correcta,
																			// siempre va a ser errónea y si es la
																			// correcta, se añadirá en el else
						...
						Collections.shuffle(idrespuestas); // Mezclo las respuestas, por si he añadido la correcta que
															// no esté siempre al final
					...
					if (puntos < 1) { // Segun los aciertos del intento, imprimo al jugador un mensaje por pantalla
						...
						System.out.println(
								"Enseñanos donde tienes la maquina del tiempo, porque lo has hecho PER-FEC-TO. ¡Enhorabuena!");
					}
				...
				case 11:
					...
					System.out.println(
							"Vas a añadir una pregunta a la base de datos. ¿Esta la respuesta correcta ya en la base de datos?");
	...
	public static int menu(String[] menu) throws InputMismatchException { // Esta función me muestra una lista de
																			// opciones y el usuario elige la opción
																			// deseada
	...
	public static int elegirOpcion(String[] menu) { // Esta función revisa que la opción elegida está contemplada en la
													// lista
	...
	public static int responderJuego(ArrayList<Integer> IDsRespuestas, int idcorrecto)
			throws ClassNotFoundException, SQLException, InputMismatchException { // Aquí muestro las respuestas y
																					// compruebo si la que elije el
																					// usuario es correcta, devolviendo
																					// un punto si es así y 0 si falla
